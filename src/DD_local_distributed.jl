...
# are done in parallel on each MPI rank using Distributed.jl.
#
# Usage:
#   DD_local_distributed.jl -i <parms file>
#
    ...
    @time "      DD_local_contracton!" begin
        ...
        contraction = (τ_charm, τ, x_sink, v_sink) -> begin
            PC.DD_local_contractons(
                τ_charm, τ, (x_sink, x_src_μiₓ_t₀, v_sink, v_src_ciₓk_t₀), Γ_arr, p_arr
            )
        end
