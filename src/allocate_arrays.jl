...
Allocate the empty arrays `x_sink_μiₓ`, `x_src_μiₓt`, `v_sink_ciₓkt`, `v_src_ciₓkt` with
...
function allocate_sparse_modes(sparse_modes_file=nothing; N_points=nothing)
    if !isnothing(sparse_modes_file)
        ...
        _, N_points = size(hdf5_file["sparse_space_sink"])
        close(hdf5_file)
    ...
    # Allocate arrays
    x_sink_μiₓ = Array{Int32}(undef, 3, N_points)
