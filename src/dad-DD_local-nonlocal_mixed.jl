...
#
# Usage:
#   dad-DD_local-nonlocal_mixed.jl -i <parms file> --nranks-per-cnfg <n>
#
...
#= correlators_nonlocal_local = zeros(
    ComplexF64, PC.parms.Nₜ, PC.parms.N_src, PC.parms.N_cnfg, length(Iₚ_nonlocal_arr)
)
correlators_local_nonlocal = zeros(
    ComplexF64, PC.parms.Nₜ, PC.parms.N_src, PC.parms.N_cnfg, length(Iₚ_nonlocal_arr)
) =#
    ...
    #= if my_cnfg_rank == 0
        ...
        for (iₚ_nonlocal, Iₚ_nonlocal) in enumerate(Iₚ_nonlocal_arr)
            ...
            PC.DD_dad_nonlocal_local_mixed_contractons!(
                ...
                Γ₁_dad_arr, Γ₂_dad_arr, Γ_DD_arr, t₀, Iₚ_nonlocal, [Ptot]
            )
        end
    ...
    for (i_cnfg, n_cnfg) in enumerate(PC.parms.cnfg_numbers)
        ...
        @time "Finished configuration $n_cnfg" begin
            ...
            for (i_src, t₀) in enumerate(PC.parms.tsrc_arr[i_cnfg, :])
                ...
                #= correlators_nonlocal_local[:, i_src, i_cnfg, :] = 1/3*(
                    C_nonlocal_local_tnmn̄m̄iₚIₚ[:, 2, 1, 1, 1, 1, :] +
                    C_nonlocal_local_tnmn̄m̄iₚIₚ[:, 3, 1, 2, 1, 1, :] +
                    C_nonlocal_local_tnmn̄m̄iₚIₚ[:, 4, 1, 3, 1, 1, :]
                )

                correlators_local_nonlocal[:, i_src, i_cnfg, :] = 1/3*(
                    C_local_nonlocal_tnmn̄m̄iₚIₚ[:, 1, 1, 2, 1, 1, :] +
                    C_local_nonlocal_tnmn̄m̄iₚIₚ[:, 2, 1, 3, 1, 1, :] +
                    C_local_nonlocal_tnmn̄m̄iₚIₚ[:, 3, 1, 4, 1, 1, :]
                ) =#
...
p₁, p₂ = [0, 0, 0], [0, 0, 0]
iₚ_nonlocal = findfirst(Iₚ -> PC.parms.p_arr[Iₚ] == [p₁, p₂] , Iₚ_nonlocal_arr)

correlator_nl_l = -correlators_nonlocal_local[:, :, :, iₚ_nonlocal]
correlator_l_nl = -correlators_local_nonlocal[:, :, :, iₚ_nonlocal]
...
plot_correlator!(correlator_nl_l, label="- Correlator nonlocal-local")
plot_correlator!(correlator_l_nl, label="- Correlator local-nonlocal")
display(plot) =#
